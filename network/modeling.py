from network.hrnet_ocr import HighResolutionNet
...
from .refineNetContrastive import rf101_contrastive
from .momentumNet import MomentumNet
from .deeplabv2 import DeepLabV2_ResNet101
...
def _segm_xception(name, backbone_name, num_classes, output_stride, pretrained_backbone):
    ...
    backbone = xception.xception(pretrained=pretrained_backbone, output_stride=output_stride)

    inplanes = 2048
    low_level_planes = 256
    
    if name=='deeplabv3plus':
        # return_layers = {'block21': 'out', 'lowfeature_layer': 'low_level'}
        classifier = DeepLabHeadV3Plus(inplanes, low_level_planes, num_classes, aspp_dilate)
    elif name=='deeplabv3':
        # return_layers = {'block21': 'out'}
        classifier = DeepLabHead(inplanes , num_classes, aspp_dilate)
    # backbone = IntermediateLayerGetter(backbone, return_layers=return_layers)
...
def getRefineNet(num_classes):
    return rf101(num_classes,True)

def getHRNet(num_classes):
    # return HRNet(cfg=cfg, NUM_CLASSES=num_classes)
    return HighResolutionNet(NUM_CLASSES=num_classes)

def getMomentum(baseNet,teacherNet,momentum=0.99):
    return MomentumNet(baseNet,teacherNet,momentum=momentum)
...
def deeplabv2_resnet101(num_classes=19, output_stride=8):
    """Constructs a DeepLabV3+ model with a ResNet-101 backbone.
    ...
    Args:
        ...
        pretrained_backbone (bool): If True, use the pretrained backbone.
    """
    return DeepLabV2_ResNet101(num_classes=num_classes)
