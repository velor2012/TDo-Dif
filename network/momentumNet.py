...
class MomentumNet(nn.Module):
    ...
    def __init__(self, baseNet, slow_copy, args):
        ...
        self.corect_num = 0
        self.refine_num = 0
        self.refine_num_extend = 0
        self.refine_num_unconf = 0
        self.total_num = 0
    ...
    def forward(self, x, \
        ...
        if update_prototype:
            ...
            if conf_plabels is not None:
                ...
                vectors, ids = self.calculate_mean_vector(feats, outputs, plabel = plabels_interp)
        ...
        if refine_outputs:
            ...
            if unconf_mask is None:
                plabels = plabels.unsqueeze(1)
                conf_plabels = conf_plabels.unsqueeze(1)
                unconf_mask = plabels == 255
            ...
            # prob_sort2 = torch.argsort(-1*soft_labels,dim=1).cpu()
            # outputs2 = torch.nn.functional.interpolate(
            #     outputs, size=soft_labels.shape[2:], mode="bilinear", align_corners=False
            # )
            # prob_sort = torch.argsort(-1*outputs2,dim=1)
            prob_sort = torch.argsort(-1*soft_labels,dim=1)
            ...
            # logits_mask2 前五类的位置为1，其他位置为0
            logits_mask2 = logits_mask1.clone()
            logits_mask2 = logits_mask2.scatter_(0,pred4.unsqueeze(dim=0),1)
            logits_mask2 = logits_mask2.scatter_(0,pred5.unsqueeze(dim=0),1)
            t = unconf_mask.squeeze(dim=1)
            # 合并两个mask
            logits_mask1[:, t] = logits_mask2[:, t]
            # t = torch.sum(logits_mask1, dim=0)
            # t2 = torch.sum(logits_mask2, dim=0)
            # t = torch.unique(t)

            logits_mask1 = logits_mask1.permute(1,0,2,3)
            # logits_mask2 = logits_mask2.permute(1,0,2,3)
            preds_before_process = torch.argmax(soft_labels, dim=1).detach().cpu().numpy()

            mask_ignore = torch.zeros(plabels.shape[0],plabels.shape[-2], plabels.shape[-1], dtype=bool)
            mask_ignore2 = torch.zeros(plabels.shape[0],plabels.shape[-2], plabels.shape[-1], dtype=bool)
            # mask2 = extend_mask.repeat(1, soft_labels.shape[1], 1, 1)
            # unconf_mask = unconf_mask.repeat(1, soft_labels.shape[1], 1, 1)
            after_prob = soft_labels * score_prot_conf * logits_mask1

            # normalize
            after_prob =  after_prob / torch.sum(after_prob, dim=1).unsqueeze(dim=1)
            # kkk = torch.sum(after_prob, dim=1)

            # after_pred = torch.argmax(after_prob, dim=1).detach().cpu().numpy()
            ignore_class_before = [0, 1, 2, 3, 4]
            # ignore_class_after =  [4, 9, 10 ,11, 14, 15, 16, 17]
            # for cc in ignore_class_after:
            #     mask_ignore |= (torch.from_numpy(after_pred) == cc)
            for cc in ignore_class_before:
                mask_ignore2 |= (torch.from_numpy(preds_before_process) == cc)

            # mask_remain = mask_ignore & mask_ignore2
            mask_remain = mask_ignore2
            mask_remain = mask_remain.unsqueeze(1).cuda()
            m = (extend_mask | unconf_mask) & mask_remain
            # t = torch.where(m)
            m = m.repeat(1, soft_labels.shape[1], 1, 1)
            soft_labels[m] = (after_prob)[m]
            del after_prob
            ...
            # # 统计置信度第j类转为第1大类的正确率
            # prob_sort2 = prob_sort2.permute(1,0,2,3)
            # torch_preds_after_process = torch.argmax(soft_labels, dim=1).detach().cpu()
            # c = prob_sort2 == torch_preds_after_process.unsqueeze(dim=0)
            # aaa = ((prob_sort2 + 1) * c).cpu().numpy()
            ...
            for i in range(len(preds_before_process)):
                ...
                self.refine_num += np.sum(mask)
                self.refine_num_unconf += np.sum(mask & unconf_mask[i][0].cpu().numpy())
                self.refine_num_extend += np.sum(mask & extend_mask[i][0].cpu().numpy())
                # 修改分母为整张图片的像素点
                self.total_num += mask.shape[0] * mask.shape[1]
                label = None
                if labels is not None:
                    label = labels[i]
                ...
                #         mask_change_j = mask2 & (pred_before_process == j) & \
                #              (pred_after_process == k) & (label == k)
                #         self.change_correct_mask[j][k] += np.sum(mask_change_j)
                #         mask_change_total = mask2 & (pred_before_process == j) & \
                #              (pred_after_process == k)
                ...
                # plt.savefig('testb.png',bbox_inches='tight')
