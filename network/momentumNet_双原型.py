...
class MomentumNet(nn.Module):
    ...
    def forward(self, x, \
        ...
        if conf_plabels is not None and plabels is not None:

            # 更新原型
            plabels = plabels.unsqueeze(1).float().clone()
            conf_plabels = conf_plabels.unsqueeze(1).float().clone()
            plabels = torch.nn.functional.interpolate(plabels,
                                                    (feats.shape[2], feats.shape[3]), mode='nearest')
            conf_plabels = torch.nn.functional.interpolate(conf_plabels,
                                                    (feats.shape[2], feats.shape[3]), mode='nearest')
        ...
        if update_prototype:
            ...
            # 更新扩充的伪标签的原型
            vectors, ids = self.calculate_mean_vector(feats, outputs, plabel = conf_plabels)
            ...
            # 更新扩充的高置信度的伪标签的原型
            vectors, ids = self.calculate_mean_vector(feats, outputs, plabel = plabels, extend_mask = extend_mask)
            #vectors, ids = class_features.calculate_mean_vector_by_output(feat_cls, output, model)
            for t in range(len(ids)):
                vector, id = vectors[t].detach(), ids[t]
        ...
        if refine_outputs:
            ...
            score_prot_extend = score_prot_extend.permute(0,3,1,2)
            ...
            score_prot_conf = score_prot_conf.permute(0,3,1,2)
            # max_scores2, pseudo_labels2 = torch.max(score_prot_extend, dim=1)
            ...
            mask2 = extend_mask.repeat(1, outputs.shape[1], 1, 1)
            unconf_mask = unconf_mask.repeat(1, outputs.shape[1], 1, 1)
            # soft_labels[mask2] = (soft_labels * score_prot_conf)[mask2]
            outputs[mask2] = (outputs * ( 0.5 + 0.5 * score_prot_conf))[mask2]
            outputs[unconf_mask] = (outputs * ( 0.5 + 0.5 * score_prot_conf))[unconf_mask]
            # soft_labels[unconf_mask] = (soft_labels * score_prot_extend)[unconf_mask]
