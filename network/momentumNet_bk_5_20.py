...
class MomentumNet(nn.Module):
    ...
    def forward(self, x, \
        ...
        if refine_outputs:
            ...
            # kk = torch.argmax(soft_labels, dim=1).detach()
            # mmm = ((extend_mask | unconf_mask) & mask_remain).squeeze(1)
            # kk = kk[mmm]
            # ccc = torch.sum(kk == 18)
            # mmm = mmm.numpy()
            # sadf = after_pred[mmm] == 18
            # safew = np.sum(sadf)
